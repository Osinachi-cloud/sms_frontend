...
export default function AdminGradebookPage() {
  ...
  const computedTotalPages = Math.ceil(filteredRows.length / PAGE_SIZE);
  ...
  return (
    <div className="space-y-5 pb-10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {computedData ? (
          <>
            ...
            <StatCard icon={<FileText className="w-4 h-4 text-violet-500" />} label="Page" value={`${currentPage + 1} of ${computedTotalPages || 1}`} />
          </>
        ) : (
          <>
      ...
      {loading ? (
        ...
        // Raw entries view when filter is not narrowed to both class + subject
        entries.length === 0 ? (
          <Card className="border-slate-200 dark:border-slate-700/50">
            <CardContent className="p-12 text-center space-y-4">
              ...
              <p className="text-xs text-slate-400 max-w-md mx-auto">
                Select a specific class and subject to view the computed gradebook breakdown.
              </p>
            </CardContent>
          </Card>
        ) : (
        ...
        <Card className="border-slate-200 dark:border-slate-700/50">
          <CardContent className="p-12 text-center space-y-4">
            ...
            <p className="text-xs text-slate-400 max-w-md mx-auto">
              No quizzes have been selected for grading yet, or no grading scheme is configured for this subject.
            </p>
          </CardContent>
        </Card>
      ) : computedData ? (
          ...
          <CardContent className="overflow-x-auto -mx-6 px-6">
            ...
            {computedTotalPages > 1 && (
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <button
                  onClick={() => setCurrentPage((p) => Math.min(computedTotalPages - 1, p + 1))}
